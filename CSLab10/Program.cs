...
namespace CSLab10 {
    class Prism { // Модуль обчислення значень ПРЯМОЇ призми //
        ...
        public static List<double> Vector_Length(double [] x, double [] y){
            ...
            sides.Add(Math.Round(Math.Sqrt(Math.Pow(x[0]-x[1], 2) + Math.Pow(y[0]-y[1], 2)),1));
            sides.Add(Math.Round(Math.Sqrt(Math.Pow(x[1]-x[2], 2) + Math.Pow(y[1]-y[2], 2)),1));
            sides.Add(Math.Round(Math.Sqrt(Math.Pow(x[0]-x[2], 2) + Math.Pow(y[0]-y[2], 2)),1));
            return sides; }
        ...
        public static double S_area (List<double> sides) { 
            ...
            if (sides.Count() == 3 && sides[0] == sides[1] && sides[0] == sides[2]) return (Math.Pow(sides[0], 2) * Math.Sqrt(3)) / 4;
            if (sides.Count() == 3 && sides[2] == Math.Sqrt(Math.Pow(sides[0], 2) + Math.Pow(sides[1], 2))) return (sides[0] * sides[1]) / 2;            
        ...
        public static string Figure_area (List<double> sides) {            
            if (sides.Count() == 3 && sides[0] == sides[1] && sides[0] == sides[2]) return "Рівньосторонній трикутник";
            if (sides.Count() == 3 && sides[2] == Math.Sqrt(Math.Pow(sides[0], 2) + Math.Pow(sides[1], 2))) return "Прямокутний трикутник";
            return "Трикутник"; } 
        public static bool Is_allow (double value_ = 0) {
            if (value_ > 0 && value_ < 250) return true;
            return false; } } 
    class Program { // Головний модуль
        public static void Main () {             
            ...
            "\n    → C: " + sides[2] + 
            ((sides.Count() == 4) ? "\n    → D: " + sides[3] : "") +                    
            "\n    ↪ Фігура у основи: " + "[" + Prism.Figure_area(sides) + "]" + 
            "\n        ↪  Периметр: " + Math.Round(Prism.P_area(sides),2) + 
            "\n        ↪  Площа: " + Math.Round(Prism.S_area(sides),2) + "\n" + 
            "\n→ Характеристики призми:" + "\n   ↪ Тип: [Пряма]" + // Константа за т.з.
            "\n        ↪ Довжина ребер: " + Math.Round(Prism.Edge_Lenth(prism.h_),2)+             
            "\n        ↪ Площа бічної поверхні: " + Math.Round(Prism.S_edge(sides, prism.h_),2) +   
            "\n        ↪ Площа повної поверхні: " + Math.Round(Prism.S_full(sides, prism.h_),2) +   
            "\n        ↪ Об'єм призми: " + Math.Round(Prism.V_prism(sides, prism.h_),2));
            Console.WriteLine(); } } }            